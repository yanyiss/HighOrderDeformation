...
void MeshParamDialog::create_Basic_Operation_Information_Widget()
{
	...
	//add by hzb
	Curve_Deformation = new QPushButton("Perform Curve Deformation");
	mainLayout->addWidget(Curve_Deformation, main_index, 0, 1, 2); main_index += 1;

	//add by hzb
	Curve_morphing = new QPushButton("Perform Curve Morphing");
	mainLayout->addWidget(Curve_morphing, main_index, 0, 1, 2); main_index += 1;
	...
	connect(Curve_Deformation, SIGNAL(clicked()), SIGNAL(Perform_Curve_Deformation_signal()));

	connect(Curve_morphing, SIGNAL(clicked()), SIGNAL(Perform_SamplingOptimization_signal()));
}
