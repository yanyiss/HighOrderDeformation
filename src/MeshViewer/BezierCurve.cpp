...
BezierCurve::BezierCurve(int deg, double tmin, double tmax, std::vector<double>w, std::vector<Point3> controlpoints)
{
	degree = deg;
	t_min = tmin;
	t_max = tmax;
...
BezierCurve::BezierCurve(int deg, double tmin, double tmax, std::vector<Point3> controlpoints)
{
	degree = deg;
	t_min = tmin;
	t_max = tmax;
...
Point3 BezierCurve::DeCasteljau(const double t)const
{
	if (isRational)
	{
...
Point3 BezierCurve::Derivative(const double t) const
{
	if (degree < 1) return Point3(0.0, 0.0, 0.0);
	...
	{	
		...
		double W_t = Aw_t(3);
		Point3 A_t = Aw_t.head(3);
		double W = Pw(3);
		Point3 S = Pw.head(3) / W;
...
Point3 BezierCurve::Derivative2(const double t) const
{	
	if (degree < 2) return Point3(0.0, 0.0, 0.0);
	...
	if (isRational) // C(t) = A(t) / W(t); C''(t) = (A''(t)-2W'(t)*C'(t) - W''(t)C(t)) / W(t) 
	{
		...
		double W_t = Aw_t(3);
		double W = Pw(3);
		Point3 A_tt = Aw_tt.head(3);
		Point3 A_t = Aw_t.head(3);
		Point3 S = Pw.head(3) / W;
		Point3 S_t = (A_t - W_t * S) / W;
...
void BezierCurve::Split(double t, BezierCurve &left_c, BezierCurve &right_c)
{
	...
	std::vector<Point3> left_pts(degree + 1), right_pts(degree + 1);
	...
	{
		std::vector<Point3> Rw(ctrlpoints);
		for (int j = 1; j <= degree; j++)
		{
			for (int i = degree; i >= j; i--)
			{
...
bool BezierCurve::LoadControlPoints(const std::string & filename)
{
	...
	ifs >> n;
	SetDegree(n);	
	SetParameterSpan(0.0, 1.0);
	SetRational(false);
	ctrlpoints.resize(n + 1, Point3(0.0, 0.0, 0.0));
