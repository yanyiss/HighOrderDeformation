...
#ifdef _DEBUG
        if (r >= _r || r < 0) {
           printf("Invalid index in vector: %i (size = %i)", r, _r);
            return;
        }
#endif
        (*this)(r) = v;
...
public:
    ...
    void reshape(int nbRows, int nbColumns)
    {
        ...
        if (nbRows * nbColumns != _r * _c)
           std::printf("Invalid dense matrix reshape: total number of entries must "
    ...
    void operator+=(const fullMatrix<scalar>& other)
    {
        if (_r != other._r || _c != other._c) {
            std::printf("Cannot sum dense matrices of different sizes");
            return;
        }
        for (int i = 0; i < _r * _c; ++i) _data[i] += other._data[i];
    }
    inline scalar operator()(int i, int j) const
    {
        ...
        if (i >= _r || i < 0 || j >= _c || j < 0) {
            printf("Invalid index to access dense matrix: %i %i (size = %i %i)",
                i, j, _r, _c);
            return 0;
        }
#endif
    ...
    void copy(const fullMatrix<scalar>& a)
    {
        if (_data && !_ownData) {
            /*Msg::Error("Dense matrix copy prohibited for proxies, use setAll "
                "instead");*/
            std::cout<<"Dense matrix copy prohibited for proxies, use setAll "
                "instead"<<std::endl;;
            return;
        }
        if (_r != a._r || _c != a._c) {
    ...
    {
        if (_r != _c) {
            std::printf("In-place transposition requires a square matrix "
                "(size = %d %d)", _r, _c);
            return;
        }
        scalar t;
...
#if defined(HAVE_EIGEN)
    {
        if (_r != _c || _r != rhs._r || _r != result._r) {
            printf("Wrong sizes for dense linear system solve (size = %d %d, "
    ...
    {
        std::printf("LU factorization requires LAPACK");
        return false;
    }
#endif
    ;
...
#if defined(HAVE_EIGEN) || !defined(HAVE_LAPACK)
    {
        std::printf("LU substitution requires LAPACK");
        return false;
    }
#endif
    ;
    ...
    {
        if (_r != _c) {
            printf("Dense matrix inverse requires square matrix (size = %d %d)",
                _r, _c);
            return false;
        }
        result.resize(_r, _c);
    ...
    {
        if (_r != _c) {
            std::printf("Dense matrix inversion requires square matrix (size = %d %d)",
                _r, _c);
            return false;
        }
        EigenMat ma(_data, _r, _c);
...
#elif !defined(HAVE_LAPACK)
    {
        printf("Dense matrix inversion requires LAPACK");
        return false;
    }
#endif
    ;
...
#elif !defined(HAVE_LAPACK)
    {
        printf("Dense matrix inversion requires Eigen or LAPACK");
        return false;
    }
#endif
    ;
...
#if defined(HAVE_EIGEN)
    {
        ...
        if (es.info() != Eigen::Success) {
            printf("Eigen could not compute eigenvalues/eigenvectors");
            return false;
        }
